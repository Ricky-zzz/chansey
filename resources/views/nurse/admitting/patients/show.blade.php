            ...
            <div class="hidden md:flex flex-col md:flex-row gap-3 md:gap-6 items-start">
                <div class="shrink-0 pt-2 md:pt-4 ">
                    <div class="avatar placeholder">
                        <div class="bg-neutral text-neutral-content rounded-full w-16 h-16 md:w-28 md:h-28  flex items-center justify-center">
                            <span class="text-md md:text-3xl  font-bold">{{ strtoupper(substr($patient->first_name, 0, 1)) }}{{ strtoupper(substr($patient->last_name, 0, 1)) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 ">
                    ...
                    <div class="space-y-2 md:space-y-4 ">
                        <div>
                            ...
                            <div class="pt-1 md:pt-2  border-t border-gray-200">
                                ...
                                <p class="text-xs md:text-sm  font-mono font-bold text-white bg-secondary px-1 md:px-2  py-0.5 md:py-1  rounded border border-neutral">{{ $patient->patient_unique_id }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 md:space-y-4 ">
                        <div>
                            <h3 class="text-xs md:text-sm md:text-md text-secondary uppercase font-bold tracking-widest mb-2 md:mb-3  pb-1 border-b border-gray-200">Personal Information</h3>
                            <div class="space-y-1 md:space-y-2 md:space">
                                <div>
                                    <p class="text-xs text-gray-500 uppercase font-semibold mb-0.5">Age</p>
                                    <p class="text-xs md:text-sm font-semibold text-neutral">{{ $patient->age }} years old</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p class="text-xs md:text-sm  font-semibold text-neutral">{{ $patient->civil_status }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 uppercase font-semibold mb-0.5">Date of Birth</p>
                                    <p class="text-xs md:text-sm  font-semibold text-neutral">{{ \Carbon\Carbon::parse($patient->date_of_birth)->format('F d, Y') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2 md:space-y-4 ">
                        <div>
                            <h3 class="text-xs md:text-sm md:text-md text-secondary uppercase font-bold tracking-widest mb-2 md:mb-3  pb-1 border-b border-gray-200">Contact Information</h3>
                            <div class="space-y-1 md:space-y-2 ">
                                <div>
                                    <p class="text-xs text-gray-500 uppercase font-semibold mb-0.5">Phone Number</p>
                                    <p class="text-xs md:text-sm  font-semibold text-neutral break-all">{{ $patient->contact_number }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 uppercase font-semibold mb-0.5">Permanent Address</p>
                                    <p class="text-xs md:text-sm  font-semibold text-neutral line-clamp-2">{{ $patient->address_permanent }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 uppercase font-semibold mb-0.5">Present Address</p>
                                    <p class="text-xs md:text-sm  font-semibold text-neutral line-clamp-2">{{ $patient->address_present ?? 'Same as permanent' }}</p>
                                </div>
                                <div class="pt-1 border-t border-rose-100">
                                    <p class="text-xs text-gray-500 uppercase font-semibold mb-0.5">Emergency Contact</p>
                                    <p class="text-xs md:text-sm  font-semibold text-rose-600 break-all">{{ $patient->emergency_contact_name }} : {{ $patient->emergency_contact_number }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
