...
<div class="card-enterprise p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-xl font-bold text-slate-800">My Availability Schedule</h2>
            <p class="text-sm text-slate-500">Manage your appointment slots for patients to book</p>
        </div>
        <button onclick="create_slot_modal.showModal()" class="btn-enterprise-primary gap-2">
    ...
    <div class="card-enterprise p-4 mb-6">
        <form action="{{ route('physician.slots.index') }}" method="GET" class="flex gap-4 items-end">
            <div class="form-control flex-1">
                <label class="label">
                    <span class="label-text font-semibold text-slate-700">From Date</span>
                </label>
            ...
            <div class="form-control flex-1">
                <label class="label">
                    <span class="label-text font-semibold text-slate-700">To Date</span>
                </label>
            ...
            <button type="submit" class="btn-enterprise-info gap-2">
            ...
            <a href="{{ route('physician.slots.index') }}" class="btn-enterprise-warning gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
            ...
            <thead>
                ...
                <tbody>
                    ...
                    <tr class="hover">
                        ...
                        {{-- TIME --}}
                        <td class="font-mono text-emerald-700 font-bold">
                            ...
                            <div class="flex justify-end gap-2">
                                ...
                                @if($slot->status === 'Active' && $slot->date >= today())
                                    <form action="{{ route('physician.slots.cancel', $slot->id) }}" method="POST"
                                        ...
                                        @csrf @method('PATCH')
                                        <button type="submit" class="btn-enterprise-danger text-xs gap-1">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-10 text-gray-400 italic">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <button onclick="create_slot_modal.showModal()" class="btn-enterprise-primary text-xs mt-2">
...
<dialog id="create_slot_modal" class="modal">
    <div class="modal-enterprise">
        ...
        <h3 class="text-lg font-bold text-slate-800 mb-4">Open New Appointment Slot</h3>
        ...
        <form action="{{ route('physician.slots.store') }}" method="POST" class="space-y-4">
            ...
            <div class="form-control">
                ...
                <input type="number" name="capacity"
                       ...
                       class="input-enterprise w-full" required>
                <label class="label">
                    <span class="label-text-alt text-slate-400">How many patients can book this slot?</span>
                </label>
                @error('capacity')
                    <span class="text-red-600 text-xs mt-1">{{ $message }}</span>
                @enderror
            ...
            <div class="modal-action">
                <button type="submit" class="btn-enterprise-primary w-full gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                    </svg>
                    Create Slot
...
<div x-data="viewAppointmentsModal()" x-cloak @open-view-modal.window="open($event.detail)">
    <dialog id="view_appointments_modal" class="modal" x-ref="modal">
        <div class="modal-enterprise max-w-3xl">
            <form method="dialog">
                <button class="absolute right-4 top-4 text-slate-400 hover:text-slate-600 transition-colors">✕</button>
            </form>

            <h3 class="text-lg font-bold text-slate-800 mb-2">Booked Patients</h3>
            <p class="text-sm text-slate-500 mb-4" x-text="slotInfo"></p>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge-enterprise text-xs"
                                          :class="{
                                              'bg-blue-50 text-blue-700 border border-blue-200': app.status === 'Booked',
                                              'bg-red-50 text-red-700 border border-red-200': app.status === 'Cancelled',
                                              'bg-emerald-50 text-emerald-700 border border-emerald-200': app.status === 'Completed',
                                              'bg-slate-100 text-slate-600': !['Booked', 'Cancelled', 'Completed'].includes(app.status)
                                          }"
                                          x-text="app.status">
                                    </span>
                                </td>
